...
async function search(params) {
    ...
    if (params.originCountry == 'BR' && params.destinationCountry == 'BR') {
        ...
        url_search = 'https://portaldoagente.com.br/OnlineTravelFrameMVC/Aereo/PesquisarFlex';
    }
    ...
    while (resultado != 8) {
        ...
        let a = await request.post({
            url: url_search, jar: cookiejar, formData: { 'FiltoDeBagagem': '', 'ControleID': token }, headers: {
                'User-Agent': 'Mozilla/5.0 (X11; Linux x86_64) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/63.0.3239.84 Safari/537.36',
                'Referer': url,
            ...
            if (array[i].NomeSistema != null) {
                ...
                if (!params.default_json) {
                    for (let a in array[i].Viagens) {
                        ...
                        json[array[i].NomeSistema][array[i].Viagens[a].VooInicial + array[i].Viagens[a].Fim.Hora.substr(0, 5)] = {
                            child: value_chd,
                            adult: value,
                        };
                        json[array[i].NomeSistema][array[i].Viagens[a].Companhia + array[i].Viagens[a].VooInicial + array[i].Viagens[a].Fim.Hora.substr(0, 5)] = {
                            child: value_chd,
                            adult: value
                        };
                    ...
                    if (params.originCountry == 'BR' && params.destinationCountry == 'BR') {
                        json[array[i].NomeSistema].Viagens = array[i].Viagens;
                    } else {
                        json[array[i].NomeSistema].Viagens = array[i].FlexViagens;
                        resultado = 8;
                    }
                }
            }
