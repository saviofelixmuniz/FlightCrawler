...
async function checkAPIHealth() {
    ...
    await Requests.getRequestSuccessRate(new Date().getTime() - ONE_HOUR, new Date().getTime()).then(async function (requests) {
        let companies = Object.keys(requests);
        for (let company of companies) {
            let companyName = Formatter.capitilizeFirstLetter(company);
            let errorRate = requests[company].errored / requests[company].total;
            if (errorRate >= ERROR_TOLERANCE) {
                console.log('Company with high error rate. Sending informative email...');

                let message = Messages.ERROR_RATE_MESSAGE(companyName, errorRate, requests[company].total);
                let subject = `FlightServer: ${companyName} está com taxa de erro alta`;
...
async function renewAirportTaxInfo() {
    ...
    let toleranceDate = currentDate.getTime() - TAX_TOLERANCE;
