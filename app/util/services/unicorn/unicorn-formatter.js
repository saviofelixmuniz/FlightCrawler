...
function responseFormat (response, params, company) {
    let formatted = CONSTANTS.getBaseVoeLegalResponse(params, company);
    let goingStretchString = params.originAirportCode + params.destinationAirportCode;

    formatted["Trechos"][goingStretchString] = {
        "Semana": {},
        "Voos": parseJSON(response, params, true)
    };

    if (params.returnDate) {
        let comingStretchString = params.destinationAirportCode + params.originAirportCode;

        formatted["Trechos"][comingStretchString] = {
            "Semana": {},
            "Voos": parseJSON(response, params, false)
        };
    }
...
function parseJSON (response, isGoing) {
    function formatDate(datetime) {
        let date = datetime.split('T')[0];
        let time = datetime.split('T')[1].split(':00Z')[0];
        return date.split('-')[2] + "/" + date.split('-')[1] + "/" + date.split('-')[0] + " " + time
    }

    let flights = isGoing? response.outbound : response.inbound;
