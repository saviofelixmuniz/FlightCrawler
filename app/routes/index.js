...
var emissions = require('./flight/emissions.route');
...
rootRouter.use('/emissions', emissions);
...
rootRouter.get('/proxytest', async function proxyTest (req, res) {
    try {
        var ip = await Requester.require({company: 'any', request: {method: 'GET', url: 'https://api.ipify.org?format=json'}});
